function SoldCtrl($scope, $stateParams, sold, clients, users){
	$scope.filter = {};
	$scope.list = {};
	$scope.filter.duration = {from: null, to: null};
	$scope.filter.fast_duration = {from: null, to: null};
	$scope.filter.detailed = false;
	$scope.preload = {'sold': true, 'clients': true};
	$scope.currentPage = 1;

	sold.$promise.then(function(data){
		$scope.sold = data;
		$scope.preload.sold = false;
	});

	console.log(' Date.fromStr')
